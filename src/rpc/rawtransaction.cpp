        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (typeStr != "scripthash" && typeStr != "scripthash_replay") {
        ...
        if (typeStr == "pubkey" || typeStr == "pubkeyhash" || typeStr == "pubkeyhash_replay" || typeStr == "multisig" || typeStr == "multisig_replay" || typeStr == "multisig_data" || typeStr == "multisig_data_replay" || typeStr == "nonstandard") {
            std::vector<std::vector<unsigned char>> solutions_data;
            TxoutType which_type = Solver(script, solutions_data);
            // Uncompressed pubkeys cannot be used with segwit checksigs.
            // If the script contains an uncompressed pubkey, skip encoding of a segwit program.
            if (which_type == TxoutType::PUBKEY || which_type == TxoutType::MULTISIG || which_type == TxoutType::MULTISIG_REPLAY || which_type == TxoutType::MULTISIG_DATA || which_type == TxoutType::MULTISIG_DATA_REPLAY) {
