                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->GetBlockHash() != m_params.GetConsensus().hashGenesisBlock) {
        assert(pindex->pprev);
        pindexBIP34height = pindex->pprev->GetAncestor(m_params.GetConsensus().BIP34Height);
    } else {
        fEnforceBIP30 = false;
    }
    ...
    if (pindex->GetBlockHash() != m_params.GetConsensus().hashGenesisBlock && !WriteUndoDataForBlock(blockundo, state, pindex, m_params)) {
        return false;
    }
