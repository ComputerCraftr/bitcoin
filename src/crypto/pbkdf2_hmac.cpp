...
{
    if (password == nullptr || salt == nullptr || key_length < 1 || output == nullptr) {
        return;
    }
                             ...
                             unsigned char* output)
{
    ...
    uint8_t last_hash[CHMAC_SHA256::OUTPUT_SIZE];
    uint8_t iteration_salt[4];
    int64_t time_end = time(nullptr) + seconds_to_hash;
...
{
    if (password == nullptr || salt == nullptr || hash == nullptr) {
        return 0;
    }
